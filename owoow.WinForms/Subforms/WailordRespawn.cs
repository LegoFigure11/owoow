...
public partial class WailordRespawn : Form
{
    ...
    public WailordRespawn(MainWindow f, EncounterType tab)
    {
        ...
        f.SetControlText(string.IsNullOrEmpty(f.TB_CurrentAdvances.Text) ? "0" : f.TB_CurrentAdvances.Text.Replace(",", string.Empty), TB_Wailord_Initial);
        f.SetControlText(((TextBox)f.Controls.Find($"TB_{Tab}_NPCs", true).FirstOrDefault()!).Text, TB_Wailord_NPCs);
    ...
    private void MenuCloseTimeline_FormClosing(object sender, FormClosingEventArgs e)
    {
        ...
        MainWindow.SetControlText(TB_Wailord_NPCs.Text, (TextBox)MainWindow.Controls.Find($"TB_{Tab}_NPCs", true).FirstOrDefault()!);
    }
